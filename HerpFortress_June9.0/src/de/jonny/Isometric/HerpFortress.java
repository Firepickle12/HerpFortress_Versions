...
public class HerpFortress extends Canvas implements Runnable {
	...
	private void tick() {
		if (mouse.b0Clicked) {
			Unit unit = Unit.create(Unit.UNIT_SPY, 0);
			unit.x = mouse.x;
			unit.y = mouse.y;
			level.add(unit);
		}
		...
		if (mouse.b2Clicked) {
			//level.explode(new Rocket(new Soldier(), mouse.x, mouse.y, 0 + 5, Math.cos(Math.PI), Math.sin(Math.PI), 0),mouse.x, mouse.y, 0, 125, 23);
		}
